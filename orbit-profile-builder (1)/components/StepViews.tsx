...
// --- EDUCATION STEP ---
export const EducationStep: React.FC<{ data: Education[]; update: (d: Education[]) => void }> = ({ data, update }) => {
      ...
      <div className="animate-fade-in pb-20">
        ...
        <div className="space-y-1">
          ...
          <div className="flex gap-3 mt-6">
            <button onClick={handleSave} className="flex-1 bg-white text-black font-bold py-3.5 rounded-2xl hover:bg-gray-200 transition-colors shadow-lg shadow-white/10">Save</button>
            {data.length > 0 && <button onClick={() => setIsAdding(false)} className="px-6 py-3.5 rounded-2xl bg-white/10 hover:bg-white/20 text-white font-medium transition-colors">Cancel</button>}
          </div>
        </div>
      </div>
    );
...
// --- EXPERIENCE STEP ---
export const ExperienceStep: React.FC<{ data: Experience[]; update: (d: Experience[]) => void }> = ({ data, update }) => {
...
// --- PROJECTS STEP ---
export const ProjectsStep: React.FC<{ data: Project[]; update: (d: Project[]) => void }> = ({ data, update }) => {
  const [isAdding, setIsAdding] = useState(false);
    ...
    return (
      <div className="animate-fade-in pb-20">
        ...
        <div className="glass-panel p-5 rounded-3xl mb-6 border-white/10">
          ...
          <div className="flex gap-3 mt-6">
            <button onClick={handleSave} className="flex-1 bg-white text-black font-bold py-3.5 rounded-2xl hover:bg-gray-200 transition-colors">Save</button>
            <button onClick={() => setIsAdding(false)} className="px-6 py-3.5 rounded-2xl bg-white/10 hover:bg-white/20 text-white font-medium transition-colors">Cancel</button>
          </div>
        </div>
      </div>
...
// --- SKILLS STEP ---
export const SkillsStep: React.FC<{ data: Skill[]; update: (d: Skill[]) => void }> = ({ data, update }) => {
    ...
    <div className="animate-fade-in pb-10">
      ...
      <div className="glass-panel p-5 rounded-3xl mb-8 bg-white/5">
        <div className="space-y-1">
           ...
           <button onClick={handleAdd} className="w-full bg-white text-black font-bold py-3.5 rounded-2xl hover:bg-gray-200 transition-colors flex items-center justify-center gap-2 mt-2">
      ...
      <div className="grid grid-cols-1 gap-3">
        {data.map(skill => (
           <div key={skill.id} className="bg-white/5 rounded-2xl p-4 border border-white/10 flex justify-between items-center group">
              ...
              <button onClick={() => removeItem(skill.id)} className="text-gray-500 hover:text-red-400 p-2">
