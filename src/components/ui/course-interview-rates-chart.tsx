...
interface CourseInterviewRatesChartProps {
  data: Array<{ course: string; rate: number; total: number; interviewed: number }>
  benchmarkData?: Array<{ course: string; rate: number }>
  showBenchmark?: boolean
  className?: string
}

export function CourseInterviewRatesChart({
  data,
  benchmarkData,
  showBenchmark = false,
  className = ''
}: CourseInterviewRatesChartProps) {
  ...
  return (
    <div className={className}>
      <h2 className="text-lg font-light text-white mb-4 flex items-center gap-2 tracking-tight">
        Course Interview Rates (% Interviewed)
      </h2>
      <ResponsiveContainer width="100%" height={250}>
        <BarChart data={chartData} layout="vertical">
          ...
          <XAxis 
            type="number" 
            stroke="#94a3b8"
            label={{ value: 'Interview Rate (%)', position: 'insideBottom', offset: -5, fill: '#94a3b8' }}
          />
          <YAxis 
            dataKey="course" 
            type="category" 
          ...
          />
          <Legend />
          <Bar dataKey="rate" fill="#f59e0b" name="Interview Rate (%)" />
          {showBenchmark && (
