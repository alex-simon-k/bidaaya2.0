                    ...
                    <div className="relative bg-white/5 backdrop-blur-xl rounded-3xl p-6 border border-white/10 overflow-hidden">
                      <div className={`flex items-start gap-6 ${
                        ...
                        <div className={`flex-1 ${imagePosition === 'left' ? 'text-right' : ''}`}>
                          ...
                          {/* Credits - Match card color */}
                          <div className={`mb-3 ${imagePosition === 'left' ? 'flex justify-end' : ''}`}>
                            <div className={`inline-flex items-center gap-2 px-3 py-1.5 rounded-lg ${
                              ...
                              <Zap className={`w-4 h-4 ${
                                plan.gradient.includes('orange') ? 'text-orange-400' : 'text-purple-400'
                              }`} />
                          ...
                          {/* Price */}
                          <div className={`mb-4 ${imagePosition === 'left' ? 'text-right' : ''}`}>
                            <span className="text-2xl font-semibold text-white">£{displayPrice}</span>
                            <span className="text-sm text-white/60 ml-1">/{billingInterval === 'year' ? 'year' : 'month'}</span>
                          </div>
                          ...
                          {/* CTA Button */}
                          {isCurrentPlan ? (
                            <div className="text-center py-3 bg-white/5 backdrop-blur-xl rounded-full text-white/80 font-semibold border border-white/10">
                              Your Current Plan
                            </div>
                          ) : isDowngrade ? (
                            <button
                              onClick={handleManageSubscription}
                              className="w-full py-3 bg-white/5 backdrop-blur-xl border border-white/20 text-white rounded-full font-bold hover:bg-white/10 transition-all flex items-center justify-center gap-2"
                            >
                            ...
                            <button
                              ...
                              disabled={upgrading}
                              className={`w-full py-3 bg-gradient-to-r ${plan.gradient} text-white rounded-full font-bold hover:opacity-90 transition-all disabled:opacity-50 flex items-center justify-center gap-2`}
                            >
                              {upgrading ? 'Loading...' : `Upgrade to ${plan.displayName === 'Premium' ? 'Premium' : 'Pro'}`}
                              <ArrowRight className="h-4 w-4" />
                            </button>
                          )}
                        </div>
