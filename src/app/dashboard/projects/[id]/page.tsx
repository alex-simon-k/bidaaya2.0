...
export default function ProjectDetailPage() {
  ...
  // Show admin applications view if admin clicked the button
  if (showAdminApplications && isAdmin() && project) {
    return (
      <AdminProjectApplications 
        project={project}
        onBack={() => setShowAdminApplications(false)}
      />
    )
  }
            ...
            <div className="flex gap-2 mt-4 sm:mt-0">
              ...
              {isAdmin() && (
                <div className="flex items-center gap-3">
                  ...
                  <button
                    onClick={() => setShowAdminApplications(true)}
                    className="flex items-center gap-2 px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors"
                  >
                    <Users className="h-4 w-4" />
                    Manage Applications ({project?.applications?.length || 0})
                  </button>
                </div>
              )}
